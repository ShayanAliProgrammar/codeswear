<x-layouts.admin title="Categories">
    <div class="flex md:flex-row md:gap-2 gap-4 flex-col">
        <x-sidebar>
            ...
            <x-ui.button as='a' variant='ghost' :href="route('admin-category-add')" class="w-full justify-start">
                Add Category
            </x-ui.button>
        </x-sidebar>
            ...
            <form action="{{ route('category-edit') }}" method="post" autocomplete="off">
                ...
                <div class="max-w-lg py-10 w-full [&>*]:w-full grid place-items-center h-full mx-auto gap-3">
                    ...
                    <div class="grid gap-2" on:visible="{{ asset('build/change-slug.js') }}">
                        ...
                        <x-ui.input id="name" name='name' type="text" :value="$category->name" />
                        @error('name')
                    ...
                    <div class="grid gap-2">
                        ...
                        <x-ui.input id="slug" readonly name='slug' type="text" :value="$category->slug" />
                        @error('slug')
                            <p class="text-destructive text-sm">{{ $message }}</p>
                        @enderror
                    </div>
